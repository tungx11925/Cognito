...
import { BookOpen, CheckSquare, ChevronLeft, LayoutDashboard, Menu, X, Sparkles } from 'lucide-react';
...
export default function StudentLayout({
  ...
  const navItems = [
    { name: 'Lớp học của tôi', href: '/student/classes', icon: LayoutDashboard },
    { name: 'Bài tập & Kiểm tra', href: '/student/assignments', icon: CheckSquare },
  ];
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <Icon size={18} className={`mr-3 ${isActive ? 'text-emerald-500' : 'text-gray-400 group-hover:text-gray-600'}`} />
                {item.name}
              </Link>
            )
          })}
