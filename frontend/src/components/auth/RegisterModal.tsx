...
export default function RegisterModal({ isOpen, onClose, triggerMessage }: RegisterModalProps) {
  ...
  const { login: contextLogin, register: contextRegister, googleLogin } = useStudy();
        ...
        <div className="overflow-y-auto p-5 custom-scrollbar" style={{ maxHeight: 'calc(100vh - 80px)' }}>
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-center">
              <button 
                type="button"
                className="w-full flex items-center justify-center gap-3 py-2.5 bg-white border border-[#0D2B24]/10 rounded-xl hover:bg-gray-50 transition-colors shadow-sm"
                onClick={() => {
                  triggerMessage("Vui lòng cấu hình Google Client ID", "error");
                }}
              >
                ...
                <span className="text-[13px] font-bold text-gray-700">Tiếp tục với Google</span>
              </button>
            </div>
          </div>
          </motion.div>
